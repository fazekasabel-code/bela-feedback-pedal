...
| `bela/gen1-multicell-live/` | Same N=12/duck-safe allocator, sweep-kick removed for live playing. Bela's own browser GUI is **blank** — root-caused to this board running Bela's `master` branch while pybela's `watcher` library only supports `dev` (see phase-plan.md); fixing it means a core Bela branch switch, deferred for now — use `host/rig/multicell_monitor.py` (pybela terminal view) instead |
