...
| `bela/gen1-multicell-live/` | Same N=12/duck-safe allocator, sweep-kick removed for live playing. Bela's own browser GUI (and core `Scope`) are **blank/disconnected**, still not fully explained after real investigation — several plausible causes checked and ruled out with evidence (see phase-plan.md) — use `host/rig/multicell_monitor.py` (pybela terminal view) instead |
