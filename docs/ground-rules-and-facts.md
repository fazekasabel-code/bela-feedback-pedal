...
- **One cell per partial.** No two bound cells may sit within the keep-out radius of each other, and **that radius is the glide window** — ±50 cents, the same number this section already uses to define "the same partial moving". One radius, one meaning: if the glide can reach it, it is the same partial and one cell owns it; if it cannot, it is a different partial and may have a cell of its own. **Set to 50 cents 2026-09-17** (Abel); it had been 110, which contradicted its own stated intent — it claimed to let adjacent semitones each hold a cell, but a semitone is 100 cents, so that was exactly the case it blocked — and left a 50–110 cent band that could neither be glided onto nor allocated a cell, in which a partial beside a live incumbent went unregulated for as long as that incumbent held its slot. (Implementation residue, below ~600 Hz only: the bin-count floors differ, 2 for glide against 3 for exclusion, leaving one bin of that band. It is the FFT window's mainlobe width, not the musical rule — two cells 2 bins apart down there would sit on one peak's mainlobe.) **Added as an explicit rule 2026-09-17**, after two pairs of cells were observed doubled on one partial live (cells 0 and 11 both on 80 Hz, cells 6 and 9 both on 734 Hz). It had been implicit in a single occupancy test at candidate-gather time, which covers binding and nothing else — the glide had no occupancy check at all, so a cell that bound to a sidelobe outside the radius then walked uphill into the peak and collided with the cell already there; and the candidate list's own entries were never mutually exclusive, so two close peaks could both bind in one frame. This matters beyond tidiness: every cell detects from the raw pre-cell input, so two cells on one partial each compute the **full** correction and both apply it — the partial is cut twice, the control loop's gain is doubled, and the partial is driven well below target. Enforcement is now at all three points where the invariant can break (bind, co-bind, glide).
...
4. **Asymmetric envelopes.** Attack fast enough to beat loop growth; release slow. A jump then costs at most one attack time of unregulated growth. (§6.3, revised 2026-09-17: "fast enough" is ~300x looser than 1–5 ms implies — the cost is growth rate × attack, and slowing the attack is what lets a plucked note's transient through.)
