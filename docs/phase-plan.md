...
2. Gain staging: pickup level into Bela (target ~-12 dBFS peak on normal playing), Bela out straight into the power amp (Dayton DTA120) — no pedal in between in this build, see ground-rules §4.5 — then **fix the power amp gain permanently and write it down**. **M4 preamp staged live, 2026-09-12** — 10:30-10:45 clock position, ‑8 to ‑14 dBFS peak on normal playing, right on target; see `rig-profile.json` `gain_staging`. **Retired 2026-09-13: the M4 itself was removed from the signal path** (confirmed source of a ground loop, see ground-rules §4.4) — this measurement belongs to a rig that no longer exists in this form. The guitar is now direct into Bela's own input, unbuffered, and gain-staging has to be redone from scratch using **Bela's own input PGA** (0–59.5 dB) once the replacement passive DI box is in hand. Power amp (DTA120) gain is not yet fixed — see item 5.
