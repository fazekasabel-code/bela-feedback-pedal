"""
Phase 5 host harness: deploy bela/gen1-multicell/ to the Bela Gem, run it for a fixed
time box against the real exciter loop, fetch the recording, score it, and write a log
record. Same shape as host/rig/gen1_cell_run.py (Phase 4) / host/rig/harness_run.py
(Phase 1) -- see those modules' headers for the fuller rationale, unchanged here.
...
Usage:
    python3 -m rig.gen1_multicell_run --seconds 30 --name first-multicell-take \
...
# bela/gen1-multicell/render.cpp by hand; there is no shared source for the two yet.
CELL_PARAMS = {
    "num_cells": 4,
    "target_db": -24.0, "q": 10.0, "attack_ms": 3.0, "release_ms": 400.0,
    "max_cut_db": 30.0, "freq_lag_s": 0.02,
    "glide_bin_radius": 2, "exclusion_bin_radius": 6,
    "min_bound_hold_frames": 40, "lockout_frames": 20,
    "bind_criterion": "loudest-stable-peak (rank), not growth-rate -- see render.cpp header",
    "steal_criterion": "least-active (smallest current cut) bound cell, past its own "
                        "anti-chatter hold",
}
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        prog="rig.gen1_multicell_run",
        description="Phase 5: run bela/gen1-multicell/ on the Gem against the real exciter loop, "
                     "score the take, log it.",
    )
    p.add_argument("--name", default="gen1-multicell-take", help="run name, used in filenames")
    ...
    if not args.i_confirm_abel_present:
        print(
            "error: this deploys the Phase 5 multicell against the real exciter for "
    ...
    rec_path = logrecord.write_record(
        run_name=args.name,
        params={
            "phase": "Phase 5 multicell", "bela_project": PROJECT_NAME,
